...
export PKGNAME=gyeeta-partha
export INSTALLDIR=/opt/gyeeta/partha
...
check_processor()
{
	...
	PROCINFO=$( cat /proc/cpuinfo 2> /dev/null )
...
{
	echo -e "\nUnable to find Kernel Header package $KERNHDR for the current kernel version $(uname -r)"
	echo -e "\nThe installation will continue but you'll need to install these yourself or reboot the system and retry installing $KERNHDR package in order to use $PKGNAME.\n"
}
...
if [ -f ${INSTALLDIR}/partha ]; then
	echo -e "\n$PKGNAME seems to be already installed at ${INSTALLDIR} : Please uninstall the older package if a fresh install is needed...\n"
	exit 1
fi

if [ $# -lt 1 ]; then
	echo -e "\nUsage : $0 <partha Config file>\n\nPlease check https://gyeeta.io/docs/installation/partha_config for reference\n\n"
	exit 1
fi	
...
if [ ! -f $CFGFILE ]; then
	echo -e "\nERROR : partha Config file $CFGFILE not found...\n\nUsage : $0 <partha Config file>\n\nPlease check https://gyeeta.io/docs/installation/partha_config for reference\n\n"
	exit 1
elif [ $( egrep -c '"shyama_hosts"|"shyama_ports"|"cluster_name"' $CFGFILE 2> /dev/null ) -lt 3 ]; then	
	echo -e "\nERROR : partha Config file missing mandatory config params.\n\nPlease check https://gyeeta.io/docs/installation/partha_config for reference\n\n"
	exit 1
fi	
...
if [ $1 != "$INSTALLDIR/cfg/partha_main.json" ]; then
	cp -f $CFGFILE $INSTALLDIR/cfg/partha_main.json
	check_cmd "Config file copy"
fi

command -v systemctl > /dev/null
if [ $? -ne 0 ]; then
	echo -e "\nSystemD systemctl command not found. $PKGNAME will not auto-start after reboot. Please run the command on reboot to start : sudo -H -u gyeeta $INSTALLDIR/runpartha.sh start\n"

	sudo -H -u gyeeta $INSTALLDIR/runpartha.sh start < /dev/null
	check_cmd "Starting $PKGNAME ...\n\n"
else
	systemctl -q start $PKGNAME
	systemctl -q enable $PKGNAME

	if [ -z "$( sudo -H -u gyeeta $INSTALLDIR/runpartha.sh printpids )" ]; then
...
if [ -n "$( sudo -H -u gyeeta $INSTALLDIR/runpartha.sh printpids )" ]; then

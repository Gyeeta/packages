...
export PKGNAME=gyeeta-alertaction
export INSTALLDIR=/opt/gyeeta/alertaction
...
if [ -f ${INSTALLDIR}/runalertaction.sh ]; then
	echo -e "\n$PKGNAME seems to be already installed at ${INSTALLDIR} : Please uninstall the older package if a fresh install is needed...\n"
	exit 1
fi

if [ $# -lt 1 ]; then
	echo -e "\nUsage : $0 <alertaction Config file>\n\nPlease check https://gyeeta.io/docs/installation/alertaction_config for reference\n\n"
	exit 1
fi	
...
if [ ! -f $CFGFILE ]; then
	echo -e "\nERROR : alertaction Config file $CFGFILE not found...\n\nUsage : $0 <alertaction Config file>\n\nPlease check https://gyeeta.io/docs/installation/alertaction_config for reference\n\n"
	exit 1
elif [ $( egrep -c "CFG_SHYAMA_HOSTS|CFG_SHYAMA_PORTS" $CFGFILE 2> /dev/null ) -lt 2 ]; then	
	echo -e "\nERROR : alertaction Config file missing mandatory config params.\n\nPlease check https://gyeeta.io/docs/installation/alertaction_config for reference\n\n"
	exit 1
fi	
...
if [ $? -ne 0 ]; then
	echo -e "\nSystemD systemctl command not found. $PKGNAME will not auto-start after reboot. Please run the command on reboot to start : sudo -H -u gyeeta $INSTALLDIR/runalertaction.sh start\n"

...
export PKGNAME=gyeeta-madhava
export INSTALLDIR=/opt/gyeeta/madhava
...
if [ -f ${INSTALLDIR}/madhava ]; then
	echo -e "\n$PKGNAME seems to be already installed at ${INSTALLDIR} : Please uninstall the older package if a fresh install is needed...\n"
	exit 1
fi

if [ $# -lt 1 ]; then
	echo -e "\nUsage : $0 <Madhava Config file>\n\nPlease check https://gyeeta.io/docs/installation/madhava_config for reference\n\n"
	exit 1
fi	
...
if [ ! -f $CFGFILE ]; then
	echo -e "\nERROR : Madhava Config file $CFGFILE not found...\n\nUsage : $0 <Madhava Config file>\n\nPlease check https://gyeeta.io/docs/installation/madhava_config for reference\n\n"
	exit 1
elif [ $( egrep -c '"madhava_name"|"shyama_hosts"|"shyama_ports"|"shyama_secret"|"service_hostname"|"service_port"|"postgres_hostname"|"postgres_port"|"postgres_user"|"postgres_password"' $CFGFILE 2> /dev/null ) -lt 10 ]; then	
	echo -e "\nERROR : Madhava Config file missing mandatory config params.\n\nPlease check https://gyeeta.io/docs/installation/madhava_config for reference\n\n"
	exit 1
fi	
...
if [ $1 != "$INSTALLDIR/cfg/madhava_main.json" ]; then
	cp -f $CFGFILE $INSTALLDIR/cfg/madhava_main.json
	check_cmd "Config file copy"
fi

command -v systemctl > /dev/null
if [ $? -ne 0 ]; then
	echo -e "\nSystemD systemctl command not found. $PKGNAME will not auto-start after reboot. Please run the command on reboot to start : sudo -H -u gyeeta $INSTALLDIR/runmadhava.sh start\n"

	sudo -H -u gyeeta $INSTALLDIR/runmadhava.sh start < /dev/null
	...
	if [ -z "$( sudo -H -u gyeeta $INSTALLDIR/runmadhava.sh printpids )" ]; then
...
if [ -n "$( sudo -H -u gyeeta $INSTALLDIR/runmadhava.sh printpids )" ]; then
